                        ...
                        Some(buzz_event) => {
                            ...
                            if accepted && queue.is_scope_in_flight(&session_scope) {
                                ...
                                if let Some(signal) = signal {
                                    ...
                                    if !native_attempted {
                                        // Scope-exact so a mid-turn steer for
                                        // this event's thread never interrupts a
                                        // different thread in the same channel.
    ...
    async fn indeterminate_project_context_requeues_without_poisoning_agent_or_circuit() {
        ...
        assert_eq!(
            returned
                .state
                .sessions
                .get(&scope::SessionScope::Conversation { channel_id })
                .map(String::as_str),
